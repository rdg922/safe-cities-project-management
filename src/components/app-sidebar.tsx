...
import { ultraFastInvalidatePermissionCaches, ultraFastFileCreationInvalidation, ultraFastInvalidateFileCaches,
    smartInvalidateFileCaches, rebuildFileCaches } from '~/lib/streamlined-cache-invalidation'
...
import { Sidebar, SidebarContent, SidebarFooter, SidebarGroup, SidebarGroupAction, SidebarGroupContent,
    SidebarGroupLabel, SidebarHeader, SidebarMenu, SidebarMenuButton, SidebarMenuItem, SidebarSeparator, SidebarTrigger } from '~/components/ui/sidebar'
                            ...
                            {isFileTreeLoading ? (
                                ...
                                <div className="px-3">
                                    <FileTree
                                                ...
                                                if (selectedFileIds.includes(dragId) && selectedFileIds.length > 1) {
                                                    ...
                                                    const promises =
                                                        selectedFileIds.map((id) =>
                                                            ...
                                                            new Promise(
                                                                (resolve, reject) => {const itemNode = findNodeById(fileTree, id)
                                                                    if (itemNode && !isParentOf(itemNode, dropNode)) {
                                                                        updateFileMutation.mutate(
                                                                            {
                                                                                id, parentId: dropNode.isFolder ? dropId: dropNode.parentId,
                                                                            },
                                                                            {
                                                                                onSuccess: resolve,
                                                                                onError: reject,
                                                    ...
                                                    const freshDragNodePromise = utils.files.getById.fetch({ id: dragId })
                                                    const freshDropNodePromise = utils.files.getById.fetch({ id: dropId })

                                                    // Process the move once we have fresh node data
                                                    Promise.all([ freshDragNodePromise, freshDropNodePromise ])
                                                        .then(
                                                            ([ dragNodeData, dropNodeData ]) => {
                                                                const freshDragNode = dragNodeData
                                                                const freshDropNode = dropNodeData

                                                                if (!freshDragNode || !freshDropNode) {
                                                                    console.error('Missing node data for drag and drop operation')
                                                                    return
                                                                }
                                                                ...
                                                                if (freshDragNode.parentId !== null) {
                                                                    fileTreeCache.clearFileCache(freshDragNode.parentId)
                                                                }

                                                                // Check if drop target is a folder or programme
                                                                if (freshDropNode.isFolder || freshDropNode.type === 'programme') {
                                                                    console.log(`Moving node ${dragId} to folder/programme ${dropId} (previous parent: ${freshDragNode.parentId})`)

                                                                    // If dropped on a folder, make it a child of that folder
                                                                    updateFileMutation.mutate({ id: dragId, parentId: dropId },
                                                                        { onSuccess: () => {
                                                                                // Perform full cache rebuild to ensure consistency
                                                                                rebuildFileCaches(utils).then( () => {
                                                                                        // Refresh UI
                                                                                        refetchFileTree()
                                                                                        console.log(`Successfully moved node ${dragId} to parent ${dropId}`)
                                                                                    }
                                                                                )
                                                                            },
                                                                        }
                                                                    ...
                                                                    const newParentId = freshDropNode.parentId ?? null
                                                                    console.log(`Moving node ${dragId} to be sibling with ${dropId} under parent ${newParentId ?? 'root'}`)

                                                                    // Clear cache for the new parent as well
                                                                    if (newParentId !== null) {
                                                                        fileTreeCache.clearFileCache(newParentId)
                                                                    }
                                                                        ...
                                                                        { id: dragId, parentId: newParentId},
                                                                        {
                                                                            onSuccess: () => {
                                                                                ...
                                                                                rebuildFileCaches(utils).then( () => {
                                                                                    // Refresh UI
                                                                                    refetchFileTree()
                                                                                    console.log(`Successfully moved node ${dragId} to parent ${newParentId ?? 'root'}`)
                                                                                    }
                                                                                )
                                                                            },
                                                                        }
                                                        ...
                                                        .catch((error) => {
                                                            ...
                                                            toast({
                                                                ...
                                                                description: 'Could not get the latest file information. Please try again.',
                                                                variant: 'destructive',
                                                            })
                                                        })
                                                }
                                        ...
                                        onCreateFile={(parentId: number | null) => {
                                            setNewFileDialogType('page')
                                            setNewFileParentId(parentId)
                                            setIsNewFileDialogOpen(true)
                                        }}
                                        onCreateSheet={(parentId: number | null) => {
                                            setNewFileDialogType('sheet')
                                            setNewFileParentId(parentId)
                                            setIsNewFileDialogOpen(true)
                                        }}
                                        onCreateForm={(parentId: number | null) => {
                                            setNewFileDialogType('form')
                                            setNewFileParentId(parentId)
                                            setIsNewFileDialogOpen(true)
                                        }}
                                        onCreateUpload={(parentId: number | null) => {
                                        ...
                                        onRename={(id: number, filename: string) => {
