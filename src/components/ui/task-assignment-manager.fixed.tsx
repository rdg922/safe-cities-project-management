...
export function TaskAssignmentManager({
    ...
    const performDatabaseSync = useCallback(async () => {
        ...
        try {
            // Sync users if changed - clear and reassign to avoid duplicates
            if (users !== undefined) {
                const userIds = users.map((user) => user.id)

                // Use a transaction-like approach: clear existing assignments first, then add new ones
                await assignTaskMutation.mutateAsync({
                    ...
                    replaceExisting: true, // Flag to indicate we want to replace all existing assignments
                })
            }
